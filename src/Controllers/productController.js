...
const createProduct = async function (req, res) {
    try {
        ...
        if (await productModel.findOne({ name })) return res.status(400).send({ status: false, message: `This name ${title} is already present please Give another name` })
        ...
        if(!category) return res.status(400).send({ status: false, message: "Please choose category"});
